function AzAPICall {
    ...
    do {
            ...
            if ($rawException.tostring() -eq 'No such host is known.' -or $rawException.tostring() -eq 'Connection timed out') {
                ...
                try {
                    ...
                    if ($targetEndpoint -eq 'Storage' -and $catchResult -like '*InvalidAuthenticationInfoServer*The token is expired.') {
                        ...
                        Write-Host "saTokenRefreshed0 $($saTokenRefreshed)" -ForegroundColor 'DarkYellow'
                        createBearerToken -targetEndPoint $targetEndpoint -AzAPICallConfiguration $AzAPICallConfiguration

                        $saTokenRefreshed = $true
                        $notTryCounter = $true
                        Write-Host "saTokenRefreshed1 = $($saTokenRefreshed); `$notTryCounter = $notTryCounter" -ForegroundColor 'DarkYellow'
                    }
        ...
        if ($saTokenRefreshed) {
            Write-Host "saTokenRefreshed $($saTokenRefreshed)" -ForegroundColor 'DarkYellow'
            $saTokenRefreshed = $false
        }
        else {
